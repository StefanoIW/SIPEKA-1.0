...
<div class="navbar-pendidikan" id="navbar-guru">
    ...
    <div class="container">
        ...
        <h3>1. Guru dapat berperan serta dalam kepanitiaan kegiatan pusat. SK atau sertifikat ditandatangani oleh Manajemen.</h3>
        <table>
            <tr>
                <th>No</th>
                <th>Nama</th>
                <th>Romawi</th>
                <th>SK / sertifikat kepanitiaan dari Manajemen</th>
                <th>Skor Sistem</th>
                <th>Skor Fix</th>
            </tr>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasiva1'];
            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $row['nama']; ?></td>
                    <td><?php echo $row['iva1']; ?></td>
                    <td>
                    ...
                    <td><?php echo $row['poiniva1']; ?></td>
                    <td>
                        <input type="text" class="editable-iva1" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoiniva1']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-iva1").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skoriva1.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
