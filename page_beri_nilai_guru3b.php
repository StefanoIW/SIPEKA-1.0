...
<body>
    ...
    <h3>B. Sharing Internal</h3>
        <h3>• 1. 1. Sharing internal dilakukan oleh guru dalam sebuah forum pertemuan. </h3>
        <table>
            <tr>
                <th>No</th>
                <th>Nama</th>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasiiib11'];
                $file_berkasiaaa = $row['berkasiiib111'];
                $file_berkasiaaaa = $row['berkasiiib1111'];
                $file_berkasiaaaaa = $row['berkasiiib11111'];
            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $row['nama']; ?></td>
                    <td><?php echo $row['iiib1']; ?></td>
                    <td>
                    ...
                    <td><?php echo $row['poiniiib1']; ?></td>
                    <td>
                        <input type="text" class="editable-iiib1" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoiniiib1']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-iiib1").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skoriiib1.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
