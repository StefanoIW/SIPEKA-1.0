...
<body>
    ...
    <div class="container">
        <h2>D. Tim Pengembang Institusi</h2>
        <h3>•1. Guru dapat berperan dalam pengembangan institusi Nusaputera secara luas.
        Surat tugas / SK ditandatangani oleh Manajemen.</h3>
        <table>
            <tr>
                <th>No</th>
                <th>Nama</th>
                <th>Romawi</th>
                <th>Surat tugas personal /SK Kepanitiaan yang ditandatangani oleh  Manajemen:</th>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasivd1'];

            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $row['nama']; ?></td>
                    <td><?php echo $row['ivd1']; ?></td>
                    <td>
                    ...
                    <td><?php echo $row['poinivd1']; ?></td>
                    <td>
                        <input type="text" class="editable-ivd1" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoinivd1']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-ivd1").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skorivd1.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
