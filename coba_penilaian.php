...
<head>
    ...
    <title>Penilaian Pendidikan</title>
    <style>
        html, body {
            height: 100%;
            margin: 0;
...
<table>
    ...
    <?php 
    $no = 1;
    $data = mysqli_query($koneksi, "SELECT * FROM tabel_romawi");
    while ($d = mysqli_fetch_array($data)) {
    ?>
    <tr>
        
        <td><?php echo $d['no']; ?></td>
        <td><?php echo $d['romawi']; ?></td> <!-- Menampilkan kolom TIMESTAMP -->
        <td><?php echo $d['nama']; ?></td>
        
        <td>
            <a href="page_beri_nilai_berkas_pendidikan.php?id_penilaian=<?php echo $d['id_penilaian']; ?>">
                <button class="btn btn-success"><strong>Beri Nilai</strong></button>
            </a>
        </td>
    </tr>
    <?php 
