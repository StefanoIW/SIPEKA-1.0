...
<body>
    ...
    <div class="container">
        <h2>B. Kepanitiaan PPDB</h2>
        <h3>• 1. Keterlibatan dalam kepanitiaan PPDB di jenjang masing2. SK diberikan oleh pimpinan jenjang ybs.</h3>
        <table>
            <tr>
                <th>No</th>
                <th>Nama</th>
                <th>Romawi</th>
                <th>SK PPDB dari pimpinan jenjang:</th>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasivb1'];
                
            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $row['nama']; ?></td>
                    <td><?php echo $row['ivb1']; ?></td>
                    <td>
                    ...
                    <td><?php echo $row['poinivb1']; ?></td>
                    <td>
                        <input type="text" class="editable-ivb1" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoinivb1']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-ivb1").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skorivb1.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
