...
<body>
    ...
    <div class="container">
        <h2>C. Peran Serta di Jenjang</h2>
        <h3>• 1. Dalam bentuk kegiatan (akreditasi, PKKS, pembukaan prodi baru, outingclass,PAS,ASAS) maupun struktural</h3>
        <table>
            <tr>
                <th>No</th>
                <th>Nama</th>
                <th>Romawi</th>
                <th>Surat tugas /SK kepanitiaan yang ditandatangani oleh pimpinan jenjang :</th>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasivc1'];

            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $row['nama']; ?></td>
                    <td><?php echo $row['ivc1']; ?></td>
                    <td>
                    ...
                    <td><?php echo $row['poinivc1']; ?></td>
                    <td>
                        <input type="text" class="editable-ivc1" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoinivc1']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-ivc1").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skorivc1.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
