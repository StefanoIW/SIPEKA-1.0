...
<div class="navbar-pendidikan" id="navbar-guru">
    ...
    <div class="container">
        <h2>D. Mengikuti lomba sesuai kompetensinya</h2>
        <h3>1. Guru mengikuti lomba sesuai kompetensinya</h3>
        <table>
            <tr>
                <th>No</th>
                <th>Nama</th>
                ...
                <th>surat tugas</th>
                <th>Flyer</th>
                <th>Dokumen pendaftaran lomba atau sertifikat keikutsertaan lomba</th>
                <th>Skor Sistem</th>
                <th>Skor Fix</th>
            </tr>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasid1'];
                $file_berkasiaaa = $row['berkasid11'];
                $file_berkasiaaaa = $row['berkasid111'];
            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    ...
                    <td><?php echo $row['poinid1']; ?></td>
                    <td>
                        <input type="text" class="editable-id1" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoinid1']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-id1").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skorid1.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
        ...
        <h3>2.Guru mengikuti lomba sesuai kompetensinya</h3>
        <table>
            <tr>
                <th>No</th>
                <th>Nama</th>
                ...
                <th>Flyer</th>
                <th>Dokumen pendaftaran lomba atau sertifikat keikutsertaan lomba</th>
                <th>Foto dan Piagam Kejuaraaan(Jika meraih Juara)</th>
                <th>Skor Sistem</th>
                <th>Skor Fix</th>
            </tr>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasid2'];
                $file_berkasiaaa = $row['berkasid22'];
                $file_berkasiaaaa = $row['berkasid222'];
            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $row['nama']; ?></td>
                    ...
                    <td><?php echo $row['poinid2']; ?></td>
                    <td>
                        <input type="text" class="editable-id2" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoinid2']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-id2").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skorid2.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
