...
<div class="navbar-pendidikan" id="navbar-guru">
    ...
    <div class="container">
        ...
        <h3>2. Pembimbing Lomba Internal (sesuai kompetensi guru)</h3>
        <h4>Pembimbing Peserta Lomba</h4>
        <table>
            <tr>
                <th>No</th>
                <th>Nama</th>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasiia2'];
                $file_berkasiaaa = $row['berkasiia22'];
                $file_berkasiaaaa = $row['berkasiia222'];
            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $row['nama']; ?></td>
                    <td><?php echo $row['iia2']; ?></td>
                    <td>
                    ...
                    <td><?php echo $row['poiniia2']; ?></td>
                    <td>
                        <input type="text" class="editable-iia2" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoiniia2']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-iia2").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skoriia2.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
        ...
        <table>
            <tr>
                ...
                <th>Flyer atau undangan</th>
                <th>Jurnal Pembimbingan</th>
                <th>Piagam/Sertifikat Kejuaraan(Jika Juara)</th>
                <th>Skor Sistem</th>
                <th>Skor Fix</th>
            </tr>
            ...
            while ($row = mysqli_fetch_array($query_ia)) {
                $file_berkasiaa = $row['berkasiiab2'];
                $file_berkasiaaa = $row['berkasiiab22'];
                $file_berkasiaaaa = $row['berkasiiab222'];
                $file_berkasiaaaaa = $row['berkasiiab2222'];
            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $row['nama']; ?></td>
                    <td><?php echo $row['iiab2']; ?></td>
                    <td>
                        <?php if (!empty($file_berkasiaa)) { ?>
                            <a href="uploads/<?php echo $file_berkasiaa; ?>" target="_blank">Download</a>
                        <?php } else {
                    ...
                    <td><?php echo $row['poiniia2']; ?></td>
                    <td>
                        <input type="text" class="editable-iiab2" data-id="<?php echo $row['id']; ?>" value="<?php echo $row['apoiniiab2']; ?>">
                    </td>
                </tr>
            <?php } ?>
        </table>
        <script>
            $(document).ready(function() {
                $(".editable-iiab2").on("change", function() {
                    var skorFix = $(this).val();
                    var id = $(this).data("id");

                    $.ajax({
                        url: "update_skoriiab2.php",
                        type: "POST",
                        data: {
                            id: id,
                            skor_fix: skorFix
